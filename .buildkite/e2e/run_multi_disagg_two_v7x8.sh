...
# instances. Run this script on the Prefill instance and provide the Decode
# instance with either:
#
#   DECODE_HOST_IP=10.0.0.2 bash run_multi_disagg_two_v7x8.sh
#
# or:
#
#   DECODE_TPU_NAME=my-decode-v7x-8 \
#   DECODE_ZONE=us-central1-b \
#   bash run_multi_disagg_two_v7x8.sh
#
# Requirements:
...
#     port and the TPUConnector transfer/side-channel ports.
#
# Unlike run_multi_disagg.sh, which splits one indivisible v7x-16 slice into
# two roles sharing a two-process PJRT runtime, each v7x-8 instance below owns
# an independent one-process PJRT runtime and an independent one-node Ray
# control plane. KV cache data moves only through TPUConnector over the network.
...
discover_decode_host() {
  ...
  if [[ -z "${DECODE_TPU_NAME:-}" ]]; then
    echo "ERROR: Set DECODE_HOST_IP or DECODE_TPU_NAME for the independent Decode v7x-8 instance." >&2
    return 1
  fi
